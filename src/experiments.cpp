...
namespace
{
    ...
    std::optional<WaypointSnapshot> parse_waypoint_snapshot(const std::filesystem::path &path)
    {
        ...
        WaypointSnapshot snapshot;
        if (auto value = read_state_value(values, "phase"))
            snapshot.phase = *value;
        if (auto value = read_state_value(values, "status"))
            snapshot.status = *value;
        if (auto value = read_state_value(values, "experiment_index"))
        {
            if (auto parsed = parse_numeric_value<std::size_t>(*value))
                snapshot.experimentIndex = (*parsed > 0) ? (*parsed - 1) : 0;
        }
        if (auto value = read_state_value(values, "total_experiments"))
        {
            if (auto parsed = parse_numeric_value<std::size_t>(*value))
                snapshot.totalExperiments = *parsed;
        }
        if (auto value = read_state_value(values, "try_index"))
        {
            if (auto parsed = parse_numeric_value<int>(*value))
                snapshot.tryIndex = *parsed;
        }
        if (auto value = read_state_value(values, "total_tries"))
        {
            if (auto parsed = parse_numeric_value<int>(*value))
                snapshot.totalTries = *parsed;
        }
        if (auto value = read_state_value(values, "step_index"))
        {
            if (auto parsed = parse_numeric_value<std::size_t>(*value))
                snapshot.stepIndex = (*parsed > 0) ? (*parsed - 1) : 0;
        }
        if (auto value = read_state_value(values, "total_steps"))
        {
            if (auto parsed = parse_numeric_value<std::size_t>(*value))
                snapshot.totalSteps = *parsed;
        }
        if (auto value = read_state_value(values, "executed_budget"))
        {
            if (auto parsed = parse_numeric_value<int>(*value))
                snapshot.executedBudget = *parsed;
        }
        if (auto value = read_state_value(values, "target_budget"))
        {
            if (auto parsed = parse_numeric_value<int>(*value))
                snapshot.targetBudget = *parsed;
        }
        if (auto value = read_state_value(values, "num_agents"))
        {
            if (auto parsed = parse_numeric_value<int>(*value))
                snapshot.numAgents = *parsed;
        }
        if (auto value = read_state_value(values, "num_objects"))
        {
            if (auto parsed = parse_numeric_value<int>(*value))
                snapshot.numObjects = *parsed;
        }
        if (auto value = read_state_value(values, "seed"))
        {
            if (auto parsed = parse_numeric_value<int>(*value))
                snapshot.seed = *parsed;
        }
        if (auto value = read_state_value(values, "ratio_random"))
        {
            if (auto parsed = parse_numeric_value<double>(*value))
                snapshot.ratioRandom = *parsed;
        }
        if (auto value = read_state_value(values, "budget"))
        {
            if (auto parsed = parse_numeric_value<int>(*value))
                snapshot.budget = *parsed;
        }
        if (auto value = read_state_value(values, "step_time_us"))
        {
            if (auto parsed = parse_numeric_value<long long>(*value))
                snapshot.stepTimeUs = *parsed;
        }
        if (auto value = read_state_value(values, "cumulative_time_us"))
        {
            if (auto parsed = parse_numeric_value<long long>(*value))
                snapshot.cumulativeTimeUs = *parsed;
        }
        if (auto value = read_state_value(values, "output_file"))
            snapshot.outputFile = *value;
        if (auto value = read_state_value(values, "best_score"))
        {
            if (auto parsed = parse_numeric_value<double>(*value))
                snapshot.bestScore = *parsed;
        }
        if (auto value = read_state_value(values, "best_allocation"))
            snapshot.bestAllocation = parse_int_list(*value);
        if (auto value = read_state_value(values, "experiment_best_score"))
        {
            if (auto parsed = parse_numeric_value<double>(*value))
                snapshot.overallBestScore = *parsed;
        }
        if (auto value = read_state_value(values, "experiment_best_allocation"))
            snapshot.overallBestAllocation = parse_int_list(*value);
...
int main(int argc, char **argv)
{
    ...
    app.add_option("-r,--resume", resumeDir, "Chemin vers un dossier d'execution a reprendre (ex: output/experiments_...)");
